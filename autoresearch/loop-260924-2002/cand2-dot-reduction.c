...
void neon_ptq1(int n, float * GGML_RESTRICT s, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        // 4 k-block int dots kept as vectors, combined with vpaddq, one f32 reduce
        int32x4_t a0 = vdotq_s32(vdotq_s32(vdupq_n_s32(0), u0, vld1q_s8(yb[0].qs)), u1, vld1q_s8(yb[0].qs+16));
        int32x4_t a1 = vdotq_s32(vdotq_s32(vdupq_n_s32(0), u2, vld1q_s8(yb[1].qs)), u3, vld1q_s8(yb[1].qs+16));
        int32x4_t a2 = vdotq_s32(vdotq_s32(vdupq_n_s32(0), u4, vld1q_s8(yb[2].qs)), vcombine_s8(w0,w1), vld1q_s8(yb[2].qs+16));
        int32x4_t a3 = vdotq_s32(vdotq_s32(vdupq_n_s32(0), vcombine_s8(w2,w3), vld1q_s8(yb[3].qs)), vcombine_s8(w4,wq), vld1q_s8(yb[3].qs+16));
        int32x4_t sums = vpaddq_s32(vpaddq_s32(a0,a1), vpaddq_s32(a2,a3)); // [sum0,sum1,sum2,sum3]
        float d1s[4] = { GGML_CPU_FP16_TO_FP32(yb[0].d), GGML_CPU_FP16_TO_FP32(yb[1].d), GGML_CPU_FP16_TO_FP32(yb[2].d), GGML_CPU_FP16_TO_FP32(yb[3].d) };
        float32x4_t prod = vmulq_f32(vcvtq_f32_s32(sums), vld1q_f32(d1s));
        sumf += GGML_CPU_FP16_TO_FP32(x[i].d) * vaddvq_f32(prod);
    }
    *s = sumf;
}
...
int main(void) {
    ...
    for (int trial = 0; trial < 200; ++trial) {
        ...
        if (fabsf(sref-snew) > 1e-2f*(fabsf(sref)+1e-3f)) {
            if (fails < 5) printf("MISMATCH trial %d: ref=%.9g neon=%.9g diff=%.3g\n", trial, sref, snew, sref-snew);
            fails++;
        }
    }
    printf(fails ? "FAIL: %d/200 trials exceed tol\n" : "PASS: 200/200 within tolerance\n", fails);
